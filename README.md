...
This MATLAB package makes it easy to change the colour scheme (a.k.a. theme) of
the MATLAB Application display and GUI.

You can use the schemer to import a predefined colour scheme, transfer your colour
settings between installations, or even create your colour scheme.

A collection of colour schemes is available in the schemes folder.
...
Colour schemes can be easily imported by running `schemer_import` at the MATLAB
command prompt, without needing any inputs. This will open a GUI to select the
file to import the colour scheme from.

When importing a colour scheme, most of the settings will change immediately.
...
Transferring a colour scheme between MATLAB installations
--------------------------------------------------------

If you are using a personalised colour scheme in MATLAB, you may wish to save
it for yourself so you can re-implement it easily if you reinstall MATLAB.
Or you may wish to transfer your personalised colour scheme from one machine
to another. This section describes the steps relevant to either scenario.

On the source machine, run `schemer_export` to save a temporary colour scheme
file, and then transfer this file to the destination machine.

When importing the new colour scheme, instead of running `schemer_import` without
...
Reverting to the MATLAB default colour scheme
--------------------------------------------

Should you wish to revert to the set of colours that MATLAB ships with, you
...
Creating a colour scheme for others to use
-----------------------------------------

When creating a colour scheme to share with the rest of the world, it is
...
If you have made a custom colour scheme using the MATLAB GUI to pick the colours,
you can export the new colour scheme with `schemer_export`.
...
If you are converting a colour scheme designed for another editor into a MATLAB
...
hexadecimal format, or terms of R, G, B values. You will need
to convert the colours from this format into the format that MATLAB preference
...
Typically, one will want to make a colour scheme that has colours in these
languages that match the colours of the analogous MATLAB syntax.
By default, this is the behaviour that Schemer will perform. 

Without specifying any inputs to `schemer_export`, an exported colour scheme file
...
If you are designing a colour scheme and are very detail-oriented, you may like to
do the following.
...
2. Export the colour scheme with `schemer_export('tmp.prf',1)`, excluding additional
   language syntax from the export.
3. Load the color scheme with `schemer_import('tmp.prf')`, which overwrites the
...
4. Open up the panel Preferences > Editor/Debugger > Languages and inspect the
   quality of the colour scheme in the additional languages.
5. Fix or improve any perceived issues with the colour scheme in the Languages panel.
6. Export the colour scheme again, but this time with `schemer_export(3)` to include
   the customised colours for the additional languages.


### Adding your scheme to this package

If you are particularly proud of your new colour scheme (and why wouldn't you be)
...
the master copy of the colour schemes issued here as part of Schemer in the schemes
directory.
